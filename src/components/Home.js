...
export default function Home() {
    ...
    async function getData(){
        const data = await fetch("http://localhost:8080/userInfo").then((d) =>d.json());
setInfo(data);
    }
    getData()
    },[])
    ...
    <div className="users">
        {
            info.map((e) =>(
                ...
                <div className="image">
                 <img style={{width:"50px", height:"50px", borderRadius:"50%"}} src={e.imgUrl} alt="" />
                </div>
                <div className="user-email">
                 <h3 style={{marginBottom:"0px"}}>{e.name}</h3>
               
                 <h4 style={{color: "grey", margin :"0px"}}>
                    {e.email}
                 </h4>
